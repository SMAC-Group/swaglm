...
//' sawglm 
//'
//' This function runs the SWAG algorithm considering a generalized linear model with link function provided by a family object using the AIC criterion to explore dimension.
//' @name sawglm
//' @param X A numeric matrix of predictors
//' @param y A numeric vector of responses
//' @param p_max An \code{integer} that specify the maximum number of dimensions to explore
...
//' @param alpha a \code{double} representing the quantile to select models at each dimension. A value of 0.1 will select the models with a criterion smaller than the first percentile at each dimension
//' @param verbose \code{boolean} specifying the ammount of information provided to the user while running the procedure
//' @return A list of list of matrices. Each list is of the length of the maximum number of dimension explored. xxx. 
//' @export
// [[Rcpp::export]]
List swaglm(const arma::mat& X, const arma::vec& y, int p_max=2, Nullable<List> family = R_NilValue, int method = 0, double alpha=0.3, bool verbose = false, int seed  = 123) {
  ...
  // save variables identified in screening in first dimension
  ...
  int p = X.n_cols;
  int m = binomial_coefficient(variables_screening.n_elem, 2); 
  ...
  for (int dimension_model = 2; dimension_model <= p_max; ++dimension_model) {
    ...
    // If more model to explore than m, select m models to evaluate randomly
    if (res3.n_rows > m) {
...
# #     if(nrow(res3)>m){
# #       res3 = res3[1:m, ] # sample with seed
# #     }
# #     if(nrow(res3)==0){ # stop if less than one model to explore
# #       break
# #     }
# # 
...
# res2 = swag(y = y, X = X, p_max = 15, q = .3, family = binomial(), eval_func = AIC)
# res2$group
# res2$selected_group
# microbenchmark::microbenchmark(res1 = swaglm::swaglm(X = X, y = y, family = binomial(), method = 0,p_max = 15, alpha = .3, verbose = T),
#                                res2 = swag(y = y, X = X, p_max = 15, q = .3, family = binomial(), eval_func = AIC), times = 10)
# )
# # 
# # 
# # 
